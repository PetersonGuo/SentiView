...
async function retrieveFormData(id) {
	...
	return new Promise((resolve, reject) => {
		...
		request.onerror = function (event) {
			reject("Error retrieving form data: " + event.target.errorCode);
		};
	});
}
		...
		async function loadTable() {
			await axios
				...
				.then((res) => {
					...
					setPosData(
						res.data.PositiveList.map((item) => ({
							name: item[0],
							count: item[1],
						}))
					);
					setNegData(
						res.data.NegativeList.map((item) => ({
							name: item[0],
							count: item[1],
						}))
					);
			...
			<div className="fixed left-[50%] top-[50%] translate-x-[-50%] translate-y-[-50%] flex main flex-col md:flex-row space-y-2 md:space-x-2">
				{modalOpen ? (
					<>
						<div
							className={
								"md:w-[35vw] md:h-[35vw] w-[80vw] h-[40vh]"
							}
						>
							<Box
								title={title}
						...
						<div className="modal md:w-[60vw] md:h-[35vw] w-[80vw] h-[40vh] flex items-center">
							<div className="border-2 border-solid border-[#FFD178] w-full h-full m-auto box flex justify-center">
								...
								<h1
									id="info"
									className="p-[20px] text-[#FFD178] text-center h-full w-full whitespace-pre-line"
								>
									{modalText}
								</h1>
							</div>
					...
					<>
						...
						>
							<Box title="Positive" loading={loading}>
								{Array.from(posData).map((data, index) => (
						...
						>
							...
							>
								{Array.from(negData).map((data, index) => (
									<ol key={index} className="list-disc">
										<motion.li
											id="press"
											className="text-[#FFD178] font-bold hover:cursor-pointer"
